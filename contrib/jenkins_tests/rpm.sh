...
if [ -x /usr/bin/dpkg-buildpackage ]; then
    ...
    ## To avoid "dpkg-checkbuilddeps: error: Unmet build dependencies: debhelper (>= 7)" on Ubuntu24.x
    dpkg -l | grep -qw debhelper || apt-get install -y debhelper
    opt_rpm=0
else
    echo "Build rpms"
    opt_rpm=1
...
if [ $opt_checkpkg -eq 1 ]; then
    test_exec="env RPM_BUILD_NCPUS=${NPROC} PRJ_RELEASE=1 ${WORKSPACE}/contrib/build_pkg.sh -b -s -i ${WORKSPACE} -o ${rpm_dir}/dist-pkg"
