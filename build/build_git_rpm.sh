...
commitDate=`git log -1 --date=short | grep Date:  | grep -Po '\d{4}[\-]\d{2}[\-]\d{2}'` #date of last commit
commitTime=`git log -1 | grep Date:  | grep -Po '\d{2}[\:]\d{2}[\:]\d{2}'` #time of last commit
		...
		if [ $PARENT == bash ]; then
			...
			if [ "$curr_branch" == "master" ]; then
				bgate_curr_branch=$master_name
                       ...
                       ssh ork@bgate.mellanox.com mkdir -p /vma/daily/"$bgate_curr_branch"/"$folderName"
                       scp -r . ork@bgate.mellanox.com:/vma/daily/"$bgate_curr_branch"/"$folderName"
                fi
                cd -
        fi
done
...
cd ..
echoStep `pwd`
DATE=`git log -1 --date=short | grep Date:  | grep -Po '\d{4}[\-]\d{2}[\-]\d{2}'`
TIME=`git log -1 | grep Date: | awk -F " "  '{print $5}'`
cd build
...
if [ "$DAILY_MODE" == 1 ]; then #daily
	...
	if [ "$make_bullseye" == 1 ]; then
		...
		bgate_branch_folder=$branch_folder
                ...
                ssh ork@bgate.mellanox.com mkdir -p /vma/daily/"$bgate_branch_folder"/"$date"
                scp -r daily/"$branch_folder"/"$date"/* ork@bgate.mellanox.com:/vma/daily/"$bgate_branch_folder"/"$date"
	fi
fi
