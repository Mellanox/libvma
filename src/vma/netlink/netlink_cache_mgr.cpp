...
#define MODULE_NAME 		"nl_cache_mgr:"

#define nl_cache_mgr_logpanic	__log_panic
#define nl_cache_mgr_logerr		__log_err
#define nl_cache_mgr_logwarn	__log_warn
#define nl_cache_mgr_loginfo	__log_info
#define nl_cache_mgr_logdbg		__log_dbg
#define nl_cache_mgr_logfunc	__log_func
...
int nl_msg_rcv_cb(struct nl_msg *msg, void *arg) {
	nl_cache_mgr_logfunc( "---> nl_msg_rcv_cb");
...
void netlink_cache_mgr::notify_observers(netlink_event *p_new_event, e_os_network_data_event_type type)
{
	g_nl_rcv_arg.nl_cache_mgr->m_cache_lock.unlock();
	g_nl_rcv_arg.nl_cache_mgr->m_subj_map_lock.lock();
	...
	g_nl_rcv_arg.nl_cache_mgr->m_subj_map_lock.unlock();
	/* coverity[missing_unlock] */
	g_nl_rcv_arg.nl_cache_mgr->m_cache_lock.lock();
}

extern void link_event_callback(nl_object* obj) {
	netlink_cache_mgr::link_cache_callback(obj);
}
extern void neigh_event_callback(nl_object* obj) {
	netlink_cache_mgr::neigh_cache_callback(obj);
}
extern void route_event_callback(nl_object* obj) {
	netlink_cache_mgr::route_cache_callback(obj);
}

void netlink_cache_mgr::neigh_cache_callback(nl_object* obj)
{
	nl_cache_mgr_logdbg( "---> neigh_cache_callback");
	struct rtnl_neigh* neigh = (struct rtnl_neigh*)obj;
	neigh_nl_event new_event(g_nl_rcv_arg.msghdr, neigh, g_nl_rcv_arg.nl_cache_mgr);

	netlink_cache_mgr::notify_observers(&new_event, NEIGH);
...
void netlink_cache_mgr::link_cache_callback(nl_object* obj)
{
	nl_cache_mgr_logfunc( "---> link_cache_callback");
	struct rtnl_link* link = (struct rtnl_link*) obj;
	link_nl_event new_event(g_nl_rcv_arg.msghdr, link, g_nl_rcv_arg.nl_cache_mgr);

	netlink_cache_mgr::notify_observers(&new_event, LINK);

	g_nl_rcv_arg.msghdr = NULL;
	nl_cache_mgr_logfunc( "<--- link_cache_callback");
}

void netlink_cache_mgr::route_cache_callback(nl_object* obj)
{
	nl_cache_mgr_logfunc( "---> route_cache_callback");
	struct rtnl_route* route = (struct rtnl_route*) obj;
	route_nl_event new_event(g_nl_rcv_arg.msghdr, route,g_nl_rcv_arg.nl_cache_mgr);
	...
	if (info->table == ROUTE_MAIN_TABLE_ID) {
		netlink_cache_mgr::notify_observers(&new_event, ROUTE);
	}
	else {
		nl_cache_mgr_logfunc("ROUTE events from non-main route table are filtered: table_id=%d", info->table);
	}
	g_nl_rcv_arg.msghdr = NULL;
	nl_cache_mgr_logfunc( "<--- route_cache_callback");
}
...
netlink_cache_mgr::netlink_cache_mgr() :
		m_cache_neigh(NULL), m_socket_handle(NULL), m_mngr(NULL), m_cache_link(
		                NULL), m_cache_route(NULL)
{
	nl_cache_mgr_logdbg( "---> netlink_route_listener CTOR");
	g_nl_rcv_arg.subjects_map = &m_subjects_map;
	g_nl_rcv_arg.nl_cache_mgr = this;
	g_nl_rcv_arg.msghdr = NULL;
	nl_cache_mgr_logdbg( "<--- netlink_route_listener CTOR");
}
...
netlink_cache_mgr::~netlink_cache_mgr()
{
...
#ifdef HAVE_LIBNL3
	nl_cache_mgr_logdbg( "---> netlink_route_listener DTOR (LIBNL3)");
...
#else // HAVE_LINBL1
	nl_cache_mgr_logdbg( "---> netlink_route_listener DTOR (LIBNL1)");
	...
	nl_cache_mgr_logdbg( "<--- netlink_route_listener DTOR");
}

int netlink_cache_mgr::open_channel()
{
	auto_unlocker lock(m_cache_lock);
	nl_cache_mgr_logdbg("opening netlink channel");
	...
	/*
	 ...
	 if (netlink_route_group_mask & LINK) {
	 subscriptions |= (1 << (RTNLGRP_LINK - 1));
	 }
	 if (netlink_route_group_mask & ADDRESS) {
	 ...
	 if (netlink_route_group_mask & ROUTE) {
	 if (!m_preferred_family || m_preferred_family == AF_INET)
	 subscriptions |= (1 << (RTNLGRP_IPV4_ROUTE - 1));
	 if (!m_preferred_family || m_preferred_family == AF_INET6)
	 subscriptions |= (1 << (RTNLGRP_IPV4_ROUTE - 1));
	 }
	 if (netlink_route_group_mask & PREFIX) {
	 if (!m_preferred_family || m_preferred_family == AF_INET6)
	 subscriptions |= (1 << (RTNLGRP_IPV6_PREFIX - 1));
	 }
	 if (netlink_route_group_mask & NEIGH) {
	...
	if (m_socket_handle == NULL) {
		nl_cache_mgr_logerr("failed to allocate netlink handle");
		return -1;
	}
	...
	BULLSEYE_EXCLUDE_BLOCK_START
	if (!m_mngr) {
		nl_cache_mgr_logerr("Fail to allocate cache manager");
		return -1;
	}
	BULLSEYE_EXCLUDE_BLOCK_END

	nl_cache_mgr_logdbg("netlink socket is open");
	...
	if (nl_socket_set_nonblocking(m_socket_handle)) {
		nl_cache_mgr_logerr("Failed to set the socket non-blocking");
		return -1;
	}
	BULLSEYE_EXCLUDE_BLOCK_END
...
int netlink_cache_mgr::get_channel()
{
	auto_unlocker lock(m_cache_lock);
	if (m_socket_handle)
...
int netlink_cache_mgr::handle_events()
{
	m_cache_lock.lock();

	nl_cache_mgr_logfunc("--->handle_events");

	BULLSEYE_EXCLUDE_BLOCK_START
	if (!m_socket_handle) {
		nl_cache_mgr_logerr("Cannot handle events before opening the channel. please call first open_channel()");
		m_cache_lock.unlock();
		return -1;
	}
	BULLSEYE_EXCLUDE_BLOCK_END
	...
	nl_cache_mgr_logfunc("nl_recvmsgs=%d", n);
	if (n < 0)
		nl_cache_mgr_logdbg("recvmsgs returned with error = %d", n);
...
bool netlink_cache_mgr::register_event(e_os_network_data_event_type type,
...
bool netlink_cache_mgr::unregister(e_os_network_data_event_type type,
                const observer* obs)
{
	auto_unlocker lock(m_subj_map_lock);
	if (obs == NULL)
...
int netlink_cache_mgr::get_neigh(const char* ipaddr, int ifindex, netlink_neigh_info* new_neigh_info)
{
	auto_unlocker lock(m_cache_lock);
	nl_cache_mgr_logfunc("--->netlink_listener::get_neigh");
	nl_object* obj;
	rtnl_neigh* neigh;
	char addr_str[256];
	...
	BULLSEYE_EXCLUDE_BLOCK_START
	if (!new_neigh_info) {
		nl_cache_mgr_logerr("Illegal argument. user pass NULL neigh_info to fill");
		return -1;
	}
	BULLSEYE_EXCLUDE_BLOCK_END
	...
	while (obj) {
		...
		if ((addr) && (index > 0)) {
			...
			if (!strcmp(addr_str, ipaddr) && (ifindex == index)) {
				...
				nl_object_put(obj);
				nl_cache_mgr_logdbg("neigh - DST_IP:%s IF_INDEX:%d LLADDR:%s", addr_str, index, new_neigh_info->lladdr_str.c_str() );
				nl_cache_mgr_logfunc("<---netlink_listener::get_neigh");
				return 1;
			}
		}
...
void netlink_cache_mgr::neigh_timer_expired() {
	m_cache_lock.lock();

	nl_cache_mgr_logfunc("--->netlink_cache_mgr::neigh_timer_expired");
...
void netlink_cache_mgr::notify_neigh_cache_entries() {
	nl_cache_mgr_logfunc("--->netlink_cache_mgr::notify_cache_entries");
	g_nl_rcv_arg.msghdr = NULL;
