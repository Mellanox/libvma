    ...
    , m_base_name {}
{
	...
	case ACTIVE_BACKUP:
		...
		valid = verify_bond_eth_qp_creation();
		break;
	default:
...
bool net_device_val::verify_bond_eth_qp_creation()
{
...
{
	return verify_qp_creation(interface_name, IBV_QPT_RAW_PACKET);
}
...
bool net_device_val::verify_qp_creation(const char* ifname, enum ibv_qp_type qp_type)
{
	...
	cq = ibv_create_cq(p_ib_ctx->get_ibv_context(), safe_mce_sys().tx_num_wr, (void*)this, channel, 0);
	if (!cq) {
