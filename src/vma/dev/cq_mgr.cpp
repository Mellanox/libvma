...
void cq_mgr::process_cq_element_log_helper(mem_buf_desc_t* p_mem_buf_desc, vma_ibv_wc* p_wce)
{
	...
	if (p_wce->status == IBV_WC_SUCCESS) {
		cq_logdbg("wce: wr_id=%#x, status=%#x, vendor_err=%#x, qp_num=%#x, csum_ok=%d", p_wce->wr_id, p_wce->status, p_wce->vendor_err, p_wce->qp_num, (!m_b_is_rx_csum_on || vma_wc_rx_csum_ok(*p_wce)));
