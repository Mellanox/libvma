...
ring::~ring()
{
	...
	int buffer_accounting = m_tx_num_wr - m_p_buffer_pool_tx->get_free_count() - m_missing_buf_ref_count;
	ring_logdbg("Tx buffer poll: free count = %u, sender_has = %d, total = %d, %s (%d)",
			m_p_buffer_pool_tx->get_free_count(), m_missing_buf_ref_count, m_tx_num_wr,
...
void ring::create_resources(ring_resource_creation_info_t* p_ring_info, int active)
{
	...
	m_p_buffer_pool_tx = new buffer_pool(m_tx_num_wr, NULL, this);	// Passing NULL instead of m_p_ib_ctx_handler so the tx buffer pool will register memory on all devices
	BULLSEYE_EXCLUDE_BLOCK_START
...
int ring::request_notification(cq_type_t cq_type, uint64_t poll_sn)
{
	...
	if (likely(CQT_RX == cq_type)) {
		RING_TRY_LOCK_RUN_AND_UPDATE_RET(m_lock_ring_rx, m_active_cq_mgr_rx->request_notification(poll_sn));
	}
	else {
...
int ring::wait_for_notification_and_process_element(cq_type_t cq_type, int cq_channel_fd, uint64_t* p_cq_poll_sn, void* pv_fd_ready_array /*NULL*/)
{
	...
	if (likely(CQT_RX == cq_type)) {
		...
		if (p_cq_mgr != NULL) {
			RING_TRY_LOCK_RUN_AND_UPDATE_RET(m_lock_ring_rx, p_cq_mgr->wait_for_notification_and_process_element(p_cq_poll_sn, pv_fd_ready_array));
		} else {
...
void ring::mem_buf_desc_completion_with_error_tx(mem_buf_desc_t* p_tx_wc_buf_desc)
{
	...
	mem_buf_tx_release(p_tx_wc_buf_desc);
}
...
{
	ring_logfuncall("");
	RING_LOCK_AND_RUN(m_lock_ring_tx, m_p_buffer_pool_tx->put_buffers(p_mem_buf_desc));
}
...
void ring::mem_buf_tx_release(mem_buf_desc_t* p_mem_buf_desc_list, bool b_accounting/*=false*/)
{
	ring_logfuncall("");
	m_lock_ring_tx.lock();
